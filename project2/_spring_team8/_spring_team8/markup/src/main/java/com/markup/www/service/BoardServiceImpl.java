...
public class BoardServiceImpl implements BoardService {
	...
	public List<BoardDTO> getDTOList(PagingVO pgvo) {
	    ...
	    List<BoardVO> list = bdao.getList(pgvo); //board list가지고옴
	    StringBuilder bnoList = new StringBuilder(); //모든bno를 하나의 문자열로 변환
	    for (int i = 0; i < list.size(); i++) {
	        bnoList.append(list.get(i).getBno());
	        if (i < list.size() - 1) {
	            bnoList.append(",");
	        }
	    }
	    FileVO fvo = new FileVO();
	    fvo.setBnos(bnoList.toString());
	    List<FileVO> flist = fdao.getFileListToBno(fvo);//문자화된 bno가지고 해당 파일가지고오기
	    List<BoardDTO> rbdtoList = new ArrayList<>(); //리턴할 BoardDTO리스트
	    
	    for (int j = 0; j < list.size(); j++) {
	        BoardDTO rbdto = new BoardDTO();
	        rbdto.setBvo(list.get(j));
	        List<FileVO> rfvo = new ArrayList<>();
	        for (int i = 0; i < flist.size(); i++) {
	            if (list.get(j).getBno() == flist.get(i).getBno()) {
	                rfvo.add(flist.get(i));
	            }
	        }
	        rbdto.setfList(rfvo);
	        rbdtoList.add(rbdto);
