...
document.getElementById('checkBtn').addEventListener('click',()=>{
    ...
    pwcheckToServer(userData).then(result=>{
        if(result>0){
            let li=`<ul><li><span class="material-symbols-outlined">check</span> 비밀번호가 확인되었습니다.</li>`;
            li+=`<li>변경할 비밀번호를 입력해주세요.</li>`;
            li +=`<li id="pwMent"><input type="password" id="newPw" class="form-control newPw"><span id="spanment"></span></li>`;
            li +=`<li>비밀번호 재확인 <input type="password" id="pwCheck" class="form-control pwCheck"><span id="checkment"></span></li>`;
            li+=`<button type="button" class="changeBtn" disabled="disabled">변경</button></li></ul>`;
            document.getElementById('resultMent').innerHTML=li;
        }else{
            document.getElementById('resultMent').innerHTML=`비밀번호가 맞지않습니다.`;
        }
    })
})


// 이벤트 위임: 상위 요소에 이벤트를 등록하고 동적으로 생성된 요소에서 실제 이벤트 처리
// document.getElementById('resultMent').addEventListener('input', function(event) {
//     if (event.target.id === 'newPw') {
//       const password = event.target.value;
//       const regex = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^*+=-])[A-Za-z\d!@#$%^*+=-]{8,}$/;
  
//       if (regex.test(password)) {
//         // 비밀번호가 조건을 충족할 경우 실행할 코드
//         document.getElementById('spanment').innerText='';
//       } else {
//         document.getElementById('spanment').innerText= '비밀번호는 영어, 숫자, 특수문자(!,@,#,$,%,^,*,+,=,-)를 포함한 8자 이상이어야 합니다.';
//       }
//     }
//     if(event.target.id==='pwCheck'){
//       const password=event.target.value;
//       const newPw=
